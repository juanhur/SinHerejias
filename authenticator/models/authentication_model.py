    ...
    def login(self, username: str, password: str, max_concurrent_users: Optional[int]=None,
        ...
        if username:
            if isinstance(max_concurrent_users, int) and self._count_concurrent_users() > \
                max_concurrent_users - 1:
                raise LoginError('Maximum number of concurrent users exceeded')
            #if single_session and self.credentials['usernames']['logged_in']:
                #raise LoginError('Cannot log in multiple sessions')
            st.session_state['email']=self.credentials['usernames']['email']
            st.session_state['name']=self.credentials['usernames']['name']
            st.session_state['roles'] = self.credentials['usernames']['roles']
            st.session_state['authentication_status'] = True
            st.session_state['username'] = self.credentials['usernames']['username']
            #self._record_failed_login_attempts(username, reset=True)
            self.credentials['usernames']['logged_in'] = True
